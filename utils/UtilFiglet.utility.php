    ...
    class Figlet {
        ...
        public function loadFontFromPath($fontFile, $setCurrent = true) {
            ...
            if (!file_exists($fontFile)) {
                throw new \Exception("Could not load figlet font '" . ($setCurrent ? $fontFile : $this->_currentFont) . "'", 404);
            }

            $this->_fontFile = file($fontFile);

            $definitions = sscanf($this->_fontFile[0], '%5s%c %d %*d %d %d %d %d %d', $this->_signature, $this->_hardblank, $this->_height, $this->_maxWidth, $this->_defaultSmush, $this->_commentLines, $this->_rightToLeft, $this->_fontSmush);
